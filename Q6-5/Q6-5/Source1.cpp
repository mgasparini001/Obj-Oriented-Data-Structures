...
Polynomial add(Polynomial p1, Polynomial p2) {
	//see which vector is shorter and append 0s to the end
	if (p1.coeffs.size() < p2.coeffs.size()) {
		for (int i = 0; i < p2.coeffs.size() - p1.coeffs.size(); i++) {
			p1.coeffs.push_back(0);
		}
	}
	else {
		for (int i = 0; i < p1.getDegree() - p2.getDegree(); i++) {
			p2.coeffs.push_back(0);
		}
	}
	//create new vector
	vector<double> resultCoeffs;
	//pushback the result of adding p1 and p2 coeffs one at a time to new vector
	for (int i = 0; i < p1.getDegree(); i++) {
		resultCoeffs.push_back(p1.coeffs[i] + p2.coeffs[i]);
	}
	//create new poly object using the result array, and return it
